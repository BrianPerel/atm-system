...
public class DBConnector {
	...
	public DBConnector() throws SQLException { // constructor -> establishes connection and creates DB (NOTE: make sure to launch apache and mysql)
		...
		try {
			...
			// open a connection to localhost and select our database. String literal is localhost URL, username=root, pass="". If database doesn't exist, exception is caught and we just connect to the localhost without selecting a db
			...
			con = DriverManager.getConnection("jdbc:mysql://localhost/atm_database", "root", ""); // select db after creating it, if it didn't exist
	...
	public void addData(int acctNo, int pin, String balance, String acctType) {
		...
		try {
			...
			st.executeUpdate("INSERT INTO Accounts (Account_Number, Pin, Account_Balance, Account_Type) " +
				"VALUES (" + acctNo + "," + pin + "," + "\'" + balance + "\'" + "," + "\'" + acctType + "\'" + ")");
